// LCEVC replacement implementation to feed bitstresm directly into base layer via an in-memory buffer
//
...
static byte *injected_annex_b_data = 0;
static unsigned injected_annex_b_size = 0;

// External hook from LCEVC
void inject_annex_b_bitstream(byte *data, unsigned size)
{
	injected_annex_b_data = data;
	injected_annex_b_size = size;
}
...
void open_annex_b (char *fn, ANNEXB_t *annex_b)
{
	...
	annex_b->iobuffer = injected_annex_b_data;

	if (NULL == annex_b->iobuffer)
	{
		error ("open_annex_b: no injected data",500);
	}
	annex_b->is_eof = FALSE;

	annex_b->bytesinbuffer = injected_annex_b_size;
